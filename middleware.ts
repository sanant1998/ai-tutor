...
/* Routes behind the app shell. Derived from the nav so a new page is guarded
   ...
   app shell — without appearing in the sidebar. /practice/[topicId] is reached
   from a link, /parent and /teacher are role-specific, and none of the three
   belongs in every student's nav. Without the list below, a signed-out visitor
   loads the app shell on those paths and only discovers they are not signed in
   when the first fetch 401s.
